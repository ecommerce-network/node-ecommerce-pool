...
// Start API on SSL port
    if (!global.config.api.sslCert) {
        global.log('error', logSystem, 'Could not start API listening on %s port %d (SSL): SSL certificate not configured', [bindIp, config.api.sslPort]);
    } else if (!global.config.api.sslKey) {
        global.log('error', logSystem, 'Could not start API listening on %s port %d (SSL): SSL key not configured', [bindIp, config.api.sslPort]);
    } else if (!global.config.api.sslCA) {
        global.log('error', logSystem, 'Could not start API listening on %s port %d (SSL): SSL certificate authority not configured', [bindIp, config.api.sslPort]);
    } else if (!fs.existsSync(global.config.api.sslCert)) {
        global.log('error', logSystem, 'Could not start API listening on %s port %d (SSL): SSL certificate file not found (configuration error)', [bindIp, config.api.sslPort]);
    } else if (!fs.existsSync(global.config.api.sslKey)) {
        global.log('error', logSystem, 'Could not start API listening on %s port %d (SSL): SSL key file not found (configuration error)', [bindIp, config.api.sslPort]);
    } else if (!fs.existsSync(global.config.api.sslCA)) {
        global.log('error', logSystem, 'Could not start API listening on %s port %d (SSL): SSL certificate authority file not found (configuration error)', [bindIp, config.api.sslPort]);
    } else {
        var options = {
            key: fs.readFileSync(global.config.api.sslKey),
            cert: fs.readFileSync(global.config.api.sslCert),
            ca: fs.readFileSync(global.config.api.sslCA),
      ...
      var bindIp = global.config.api.bindIp ? global.config.api.bindIp : "0.0.0.0";
        ssl_server.listen(global.config.api.sslPort, bindIp, function(){
            global.log('info', logSystem, 'API started & listening on %s port %d (SSL)', [bindIp, global.config.api.sslPort]);
        });
    }
